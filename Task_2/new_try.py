...
from sklearn.model_selection import train_test_split, StratifiedKFold, ParameterGrid, GridSearchCV, KFold
...
for task in [1, 2, 3, 12, '2NN', '3NN']:
    ...
    if task == 1:
        ...
        search_space = list(ParameterGrid(task2_NN_parameters_searchspace))

    elif task == '3NN':
        search_space = list(ParameterGrid(task3_NN_parameters_searchspace))
        ...
        if not already_tried:
            ...
            if task == '2NN':
                ...
                df_submission_val, df_submission = NN_pipeline(parameters,
                                                               train_features, train_labels, test, val_features,
                                                               val_labels, task=3,
                                                               experiment_logger=experiment_logger3,
                                                               with_val_set=with_val_set)
            else:
                ...
                feature_cols = new_train_features_imp.columns.values[
                    (new_train_features_imp.columns.values != 'pid') & (
                            new_train_features_imp.columns.values != 'Time')]
                ...
                for label in tqdm(TESTS):
                    if label == 'LABEL_Sepsis' and task == 2:
                        ...
                        if 'feature_selection' in parameters.keys() and parameters['feature_selection']:
                            model = Pipeline([
                                ('feature_selection', SelectFromModel(get_feature_selector(parameters))),
                        ...
                        if experiment_logger2.previous_results_empty or val_auc_score > \
                                experiment_logger2.previous_results[
                    ...
                    if not label == 'LABEL_Sepsis' and task == 1:
                        ...
                        """
                        # if previous results empty or if current score better than best previous score write to submission and to validation_pred
                        if experiment_logger1.previous_results_empty or val_auc_score > \
                                experiment_logger1.previous_results[
                                    '{}_score'.format(label)].max():
                            predictions_prob_test = model.predict_proba(X_test)
                            predict_prob_val = pd.DataFrame(np.ravel(predictions_prob_val[:, 1]), columns=[label])
                            predict_prob_test = pd.DataFrame(np.ravel(predictions_prob_test[:, 1]), columns=[label])
                ...
                for label in tqdm(VITALS):
                    if task == 3:
                        ...
                        if task3_parameters['with_grid_search_cv']:
                            ...
                            model = GridSearchCV(estimator=model, param_grid=param_grid, cv=skf,
            ...
            if len(val_labels.columns) == len(df_submission_val.columns):
            ...
            print(get_all_scores(val_labels, val_predictions))
